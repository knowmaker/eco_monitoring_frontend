...
- `src/App.jsx` - основной экран-компоновщик.
- `src/api` - функции обмена с FastAPI-сервисом.
- `src/components` - UI-компоненты приложения.
- `src/components/auth` - окно входа и регистрации.
- `src/components/profile` - окно профиля пользователя.
- `src/components/layout` - верхняя панель и боковое меню.
- `src/components/map` - логика карты мониторинга.
- `src/components/stations` - список постов, карточка станции и форма редактирования.
- `src/components/latest-readings` - последние агрегированные показания станции.
- `src/components/readings-history` - история показаний, графики, периоды и raw drilldown.
- `src/components/raw-mqtt` - просмотр сырых MQTT-пакетов.
- `src/components/export-aggregates` - экспорт агрегированных данных.
- `src/components/charts` - общие настройки и форматирование графиков.
- `src/domain` - доменные константы постов, устройств и показаний.
- `src/hooks` - экранные хуки для авторизации, постов, устройств и адаптивного режима.
- `src/lib` - общие утилиты.
- `src/styles` - стили приложения, разделённые по зонам интерфейса.
